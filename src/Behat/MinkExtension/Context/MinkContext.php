...
class MinkContext extends RawMinkContext implements TranslatableContext
{
    ...
    /**
     * Example: When I fill in shipping data "username" with: "bwayne"
     * @When /^(?:|I )fill in shipping data "(?P<field>(?:[^"]|\\")*)" with "(?P<value>(?:[^"]|\\")*)"$/
     */
    public function fillShippingField($field, $value)
    {
        $field = $this->fixStepArgument($field);
        $value = $this->fixStepArgument($value);
        ...
        for ($i = 0; $i <= 15; $i++) {
            try {
                $this->getSession()->getPage()->fillField("shipping_step_newShippingAddress_".$field, $value);
                return;
            } catch (\Exception $e) {
    ...
    /**
     * @When /^(?:|I )select "(?P<option>(?:[^"]|\\")*)" from 'Salutation'$/
     */
    public function selectSalutation($select)
    {
        $select = $this->fixStepArgument($select);
        $this->getSession()->getPage()->selectFieldOption("shipping_step_newShippingAddress_salutation", $select);
    }
    ...
    {
        $this->spins(function() use ($element, $text) {
    ...
    /**
     *  Click with specified id
     * Example: When I click "Log In"
...

    {
        $this->spins(function() use($element){
            $button = $this->getSession()->getPage()->find('css', $element);
            if (!$button) {
    ...
    /**
     *  Click 'Buy this item'
     * Example: When I click 'Buy this item'
     * Example: And I click 'Buy this item'
     *
     * @When /^(?:|I )click 'Buy this item'$/
     */
    public function clickBuyThisItem()

    {
        $this->spins(function(){
            $button = $this->getSession()->getPage()->find('css', "button:contains('Buy this item')");
            if (!$button) {
                throw new \Exception('Button is not there yet :(');
            }
            $button->click();
     ...
     * Example: When I click 'Proceed to checkout'
     * Example: And I click 'Proceed to checkout'
     *
     * @When /^(?:|I )click 'Proceed to checkout'$/
     */
    public function clickProceedToCheckout()

    {
        $this->spins(function(){
            $button = $this->getSession()->getPage()->find('css', ".shop-footer a");
            if (!$button) {
                throw new \Exception('Button is not there yet :(');
            }
            $button->click();
    ...
    /**
     * @When /^I change currency to dollar$/
     */
    public function iChangeCurrencyToDollar() {
        $field = $this->getSession()->getPage()->find('css', '#dropdownMenu1 > span.current-currency');
        $field->click();
        $this->getSession()->getPage()->find('css', '#navigation-right > div > ul > li:nth-child(5) > a')->click();
    ...
    /**
     * @When /^I switch to frame "(?P<frame>(?:[^"]|\\")*)"$/
     */
    ...
    public function switchToWindow() {
        ...
        $this->getSession()->switchToWindow($this->getSession()->getWindowNames()[0]);
    }

    public function spins($closure)
    {
        for ($i = 0; $i <= 15; $i++) {
            try {
                $closure();
